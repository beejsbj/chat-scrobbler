  ...
  <main id="main">
    ...
    <section class="coverage" aria-labelledby="coverage-title">
      <div class="section-head">
        ...
        <p class="section-note">No costs are invented. No prompt or response text is copied into this dashboard.</p>
      </div>
      <div id="coverage-list" class="coverage-list"></div>
    </section>
  </main>
  <footer><span>local-first usage index</span><span id="devices">—</span></footer>
...
function totals(rows){ return rows.reduce((a,b)=>{a.tokens+=b.totalTokens;a.sessions+=b.sessions;a.messages+=b.messages;if(b.project!=='Unattributed'&&b.project!=='General chat')a.projects.add(b.project);return a},{tokens:0,sessions:0,messages:0,projects:new Set()}); }
function renderTotals(rows){const t=totals(rows);$('total-tokens').textContent=format(t.tokens);$('total-sessions').textContent=exact(t.sessions);$('total-messages').textContent=format(t.messages);$('total-projects').textContent=exact(t.projects.size);}
...
fetch('/api/usage',{credentials:'same-origin'}).then(r=>{if(!r.ok)throw new Error('usage API returned '+r.status);return r.json()}).then(data=>{snapshot=data;$('as-of').textContent=new Date(data.generatedAt).toLocaleString(undefined,{dateStyle:'medium',timeStyle:'short'});$('devices').textContent=(data.devices||[]).join(' + ')||'no devices';renderCoverage();render();}).catch(err=>{$('error').hidden=false;$('error').textContent='Could not load the usage ledger: '+err.message;});
`;
