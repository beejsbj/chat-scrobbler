...
test("ChatGPT captureOne uploads discovered attachment bytes into raw capture assets", async () => {
  ...
  await createChatgptAdapter(fetcher).captureOne?.("with-asset", "2026-06-05T11:00:00.000Z", {
    emitCapture: async (capture) => { captures.push(capture); },
    uploadAsset: async (asset) => {
  ...
  expect(uploadedPointers).toEqual(["file-service://file-1"]);
  expect(captures[0].assets).toEqual([expect.objectContaining({
    pointer: "file-service://file-1",
    local_path: "assets/chatgpt/with-asset/hash.png",
    content_type: "image/png",
...
test("ChatGPT captureOne throws a recognizable rate-limit error", async () => {
  ...
  const error = await createChatgptAdapter(fetcher).captureOne?.("limited", null, { emitCapture: async () => {} })
...
test("Claude captureOne uploads discovered file bytes into raw capture assets", async () => {
  const uploadedPointers: string[] = [];
  const fetcher: FetchLike = async (input) => {
    if (input === "/api/organizations") return json([{ uuid: "org-1" }]);
    if (String(input).startsWith("/api/organizations/org-1/chat_conversations/cl-asset")) {
  ...
  await createClaudeAdapter(fetcher).captureOne?.("cl-asset", "2026-06-05T11:00:00.000Z", {
    emitCapture: async (capture) => { captures.push(capture); },
    uploadAsset: async (asset) => {
      uploadedPointers.push(asset.pointer);
      return {
  ...
  expect(uploadedPointers).toEqual(["file-1"]);
  expect(captures[0].assets).toEqual([expect.objectContaining({
    pointer: "file-1",
    filename: "paper.pdf",
    local_path: "assets/claude/cl-asset/hash.pdf",
...
test("Claude captureOne throws a recognizable rate-limit error", async () => {
  ...
  const error = await createClaudeAdapter(fetcher).captureOne?.("limited", null, { emitCapture: async () => {} })
    .then(() => null, (caught) => caught);
